...
echo "Starting servers on ports $PORT1,$PORT2,$PORT3 for machine7"
...
echo "mcts vs alphazero on port $PORT1"
env CONTRAST_SERVER_PORT="$PORT1" "$CLIENT_APP" X mcts_X3 mcts "$GAMES" > "$LOGDIR/mcts_vs_alphazero_X.log" 2>&1 &
CLIENT1=$!
"$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$PORT1" --role O --name az_m7 --games "$GAMES" > "$LOGDIR/mcts_vs_alphazero_O.log" 2>&1 &
CLIENT2=$!

echo "mcts vs mcts on port $PORT2"
env CONTRAST_SERVER_PORT="$PORT2" "$CLIENT_APP" X mcts_A mcts "$GAMES" > "$LOGDIR/mcts_vs_mcts_X.log" 2>&1 &
CLIENT3=$!
env CONTRAST_SERVER_PORT="$PORT2" "$CLIENT_APP" O mcts_B mcts "$GAMES" > "$LOGDIR/mcts_vs_mcts_O.log" 2>&1 &
CLIENT4=$!

echo "rulebased2 vs mcts on port $PORT3"
env CONTRAST_SERVER_PORT="$PORT3" "$CLIENT_APP" X rulebased2_X3 rulebased2 "$GAMES" > "$LOGDIR/rulebased2_vs_mcts_X.log" 2>&1 &
CLIENT5=$!
env CONTRAST_SERVER_PORT="$PORT3" "$CLIENT_APP" O mcts_O4 mcts "$GAMES" > "$LOGDIR/rulebased2_vs_mcts_O.log" 2>&1 &
CLIENT6=$!
