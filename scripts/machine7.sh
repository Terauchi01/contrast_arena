...
VENV_DIR="${ROOT_DIR}/.venv_alphazero7"
...
LOGDIR="${ROOT_DIR}/logs/machine7"
PORT=${PORT:-18903}
GAMES=${1:-1000}
mkdir -p "$LOGDIR"

# If previous server/client processes are still running, kill them to avoid port conflicts
echo "Cleaning up existing server/client processes for machine7 (if any)"
...
echo "Running 3 matches sequentially on ports $PORT1,$PORT2,$PORT3 for machine7"
SERVER_PIDS=()
CLIENT_PIDS=()
for i in 1 2 3; do
	case "$i" in
		1)
			P=$PORT1
			echo "Match #$i: mcts vs alphazero on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X mcts_X3 mcts "$GAMES")
			CMD2=("$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$P" --role O --name az_m7 --games "$GAMES")
			LOG1="$LOGDIR/mcts_vs_alphazero_X.log"
			LOG2="$LOGDIR/mcts_vs_alphazero_O.log"
			;;
		2)
			P=$PORT2
			echo "Match #$i: mcts vs mcts on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X mcts_A mcts "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O mcts_B mcts "$GAMES")
			LOG1="$LOGDIR/mcts_vs_mcts_X.log"
			LOG2="$LOGDIR/mcts_vs_mcts_O.log"
			;;
		3)
			P=$PORT3
			echo "Match #$i: rulebased2 vs mcts on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X rulebased2_X3 rulebased2 "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O mcts_O4 mcts "$GAMES")
			LOG1="$LOGDIR/rulebased2_vs_mcts_X.log"
			LOG2="$LOGDIR/rulebased2_vs_mcts_O.log"
			;;
	esac
