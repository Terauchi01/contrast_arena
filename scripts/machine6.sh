...
echo "Running 3 matches sequentially on ports $PORT1,$PORT2,$PORT3 for machine6"
SERVER_PIDS=()
CLIENT_PIDS=()
for i in 1 2 3; do
	case "$i" in
		1)
			P=$PORT1
			echo "Match #$i: alphazero vs rulebased2 on port $P"
			CMD1=("$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$P" --role X --name az_m6 --games "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O rulebased2_O4 rulebased2 "$GAMES")
			LOG1="$LOGDIR/alphazero_vs_rulebased2_X.log"
			LOG2="$LOGDIR/alphazero_vs_rulebased2_O.log"
			;;
		2)
			P=$PORT2
			echo "Match #$i: mcts vs alphabeta on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X mcts_X2 mcts "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O alphabeta_O4 alphabeta "$GAMES")
			LOG1="$LOGDIR/mcts_vs_alphabeta_X.log"
			LOG2="$LOGDIR/mcts_vs_alphabeta_O.log"
			;;
		3)
			P=$PORT3
			echo "Match #$i: rulebased2 vs alphabeta on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X rulebased2_X2 rulebased2 "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O alphabeta_O5 alphabeta "$GAMES")
			LOG1="$LOGDIR/rulebased2_vs_alphabeta_X.log"
			LOG2="$LOGDIR/rulebased2_vs_alphabeta_O.log"
			;;
	esac
