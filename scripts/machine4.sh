...
echo "Starting servers on ports $PORT1,$PORT2,$PORT3 for machine4"
...
echo "alphabeta vs ntuple on port $PORT1"
env CONTRAST_SERVER_PORT="$PORT1" "$CLIENT_APP" X alphabeta_X2 alphabeta "$GAMES" > "$LOGDIR/alphabeta_vs_ntuple_X.log" 2>&1 &
CLIENT1=$!
env CONTRAST_SERVER_PORT="$PORT1" "$CLIENT_APP" O ntuple_O ntuple "$GAMES" > "$LOGDIR/alphabeta_vs_ntuple_O.log" 2>&1 &
CLIENT2=$!

echo "alphazero vs mcts on port $PORT2"
"$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$PORT2" --role X --name az_m4 --games "$GAMES" > "$LOGDIR/alphazero_vs_mcts_X.log" 2>&1 &
CLIENT3=$!
env CONTRAST_SERVER_PORT="$PORT2" "$CLIENT_APP" O mcts_O3 mcts "$GAMES" > "$LOGDIR/alphazero_vs_mcts_O.log" 2>&1 &
CLIENT4=$!

echo "ntuple vs ntuple on port $PORT3"
env CONTRAST_SERVER_PORT="$PORT3" "$CLIENT_APP" X ntuple_A ntuple "$GAMES" > "$LOGDIR/ntuple_vs_ntuple_X.log" 2>&1 &
CLIENT5=$!
env CONTRAST_SERVER_PORT="$PORT3" "$CLIENT_APP" O ntuple_B ntuple "$GAMES" > "$LOGDIR/ntuple_vs_ntuple_O.log" 2>&1 &
CLIENT6=$!
