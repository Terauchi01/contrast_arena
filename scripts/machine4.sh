...
VENV_DIR="${ROOT_DIR}/.venv_alphazero4"
...
LOGDIR="${ROOT_DIR}/logs/machine4"
PORT=${PORT:-18892}
GAMES=${1:-1000}
mkdir -p "$LOGDIR"

# If previous server/client processes are still running, kill them to avoid port conflicts
echo "Cleaning up existing server/client processes for machine4 (if any)"
...
echo "Running 3 matches sequentially on ports $PORT1,$PORT2,$PORT3 for machine4"
SERVER_PIDS=()
CLIENT_PIDS=()
for i in 1 2 3; do
	case "$i" in
		1)
			P=$PORT1
			echo "Match #$i: alphabeta vs ntuple on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X alphabeta_X2 alphabeta "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O ntuple_O ntuple "$GAMES")
			LOG1="$LOGDIR/alphabeta_vs_ntuple_X.log"
			LOG2="$LOGDIR/alphabeta_vs_ntuple_O.log"
			;;
		2)
			P=$PORT2
			echo "Match #$i: alphazero vs mcts on port $P"
			CMD1=("$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$P" --role X --name az_m4 --games "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O mcts_O3 mcts "$GAMES")
			LOG1="$LOGDIR/alphazero_vs_mcts_X.log"
			LOG2="$LOGDIR/alphazero_vs_mcts_O.log"
			;;
		3)
			P=$PORT3
			echo "Match #$i: ntuple vs ntuple on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X ntuple_A ntuple "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O ntuple_B ntuple "$GAMES")
			LOG1="$LOGDIR/ntuple_vs_ntuple_X.log"
			LOG2="$LOGDIR/ntuple_vs_ntuple_O.log"
			;;
	esac
