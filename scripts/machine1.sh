...
# Three matches -> run sequentially: start server, start two clients, wait, stop server
PORT1=${PORT}
PORT2=$((PORT + 1))
PORT3=$((PORT + 2))

echo "Running 3 matches sequentially on ports $PORT1, $PORT2, $PORT3 for machine1"
SERVER_PIDS=()
CLIENT_PIDS=()
for i in 1 2 3; do
	case "$i" in
		1)
			P=$PORT1
			echo "Match #$i: alphabeta (X) vs alphazero (O) on port $P"
			...
			LOG1="$LOGDIR/alphabeta_vs_alphazero_X.log"
			LOG2="$LOGDIR/alphabeta_vs_alphazero_O.log"
			;;
		2)
			P=$PORT2
			echo "Match #$i: mcts (X) vs rulebased2 (O) on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X mcts_X mcts "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O rulebased2_O rulebased2 "$GAMES")
			LOG1="$LOGDIR/mcts_vs_rulebased2_X.log"
			LOG2="$LOGDIR/mcts_vs_rulebased2_O.log"
			;;
		3)
			P=$PORT3
			echo "Match #$i: rulebased2 (X) vs alphazero (O) on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X rulebased2_X rulebased2 "$GAMES")
			CMD2=("$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$P" --role O --name alphazero_O2 --games "$GAMES")
			LOG1="$LOGDIR/rulebased2_vs_alphazero_X.log"
			LOG2="$LOGDIR/rulebased2_vs_alphazero_O.log"
			;;
	esac
