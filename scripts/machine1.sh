...
VENV_DIR="${ROOT_DIR}/.venv_alphazero1"
...
LOGDIR="${ROOT_DIR}/logs/machine1"
PORT=${PORT:-18881}
GAMES=${1:-1000}
mkdir -p "$LOGDIR"

# If previous server/client processes are still running, kill them to avoid port conflicts
echo "Cleaning up existing server/client processes for machine1 (if any)"
...
# Start three servers on consecutive ports so each match can use its own server
PORT1=${PORT}
PORT2=$((PORT + 1))
PORT3=$((PORT + 2))

echo "Starting servers on ports $PORT1, $PORT2, $PORT3 for machine1"
...
echo "Starting alphabeta (X) vs alphazero (O) on port $PORT1"
env CONTRAST_SERVER_PORT="$PORT1" "$CLIENT_APP" X alphabeta_X alphabeta "$GAMES" > "$LOGDIR/alphabeta_vs_alphazero_X.log" 2>&1 &
CLIENT1=$!
"$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$PORT1" --role O --name alphazero_O --games "$GAMES" > "$LOGDIR/alphabeta_vs_alphazero_O.log" 2>&1 &
CLIENT2=$!

echo "Starting mcts (X) vs rulebased2 (O) on port $PORT2"
env CONTRAST_SERVER_PORT="$PORT2" "$CLIENT_APP" X mcts_X mcts "$GAMES" > "$LOGDIR/mcts_vs_rulebased2_X.log" 2>&1 &
CLIENT3=$!
env CONTRAST_SERVER_PORT="$PORT2" "$CLIENT_APP" O rulebased2_O rulebased2 "$GAMES" > "$LOGDIR/mcts_vs_rulebased2_O.log" 2>&1 &
CLIENT4=$!

echo "Starting rulebased2 (X) vs alphazero (O) on port $PORT3"
env CONTRAST_SERVER_PORT="$PORT3" "$CLIENT_APP" X rulebased2_X rulebased2 "$GAMES" > "$LOGDIR/rulebased2_vs_alphazero_X.log" 2>&1 &
CLIENT5=$!
"$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$PORT3" --role O --name alphazero_O2 --games "$GAMES" > "$LOGDIR/rulebased2_vs_alphazero_O.log" 2>&1 &
CLIENT6=$!
