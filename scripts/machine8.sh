...
echo "Starting servers on ports $PORT1,$PORT2,$PORT3 for machine8"
...
echo "mcts vs ntuple on port $PORT1"
env CONTRAST_SERVER_PORT="$PORT1" "$CLIENT_APP" X mcts_X4 mcts "$GAMES" > "$LOGDIR/mcts_vs_ntuple_X.log" 2>&1 &
CLIENT1=$!
env CONTRAST_SERVER_PORT="$PORT1" "$CLIENT_APP" O ntuple_O4 ntuple "$GAMES" > "$LOGDIR/mcts_vs_ntuple_O.log" 2>&1 &
CLIENT2=$!

echo "ntuple vs alphazero on port $PORT2"
env CONTRAST_SERVER_PORT="$PORT2" "$CLIENT_APP" X ntuple_X4 ntuple "$GAMES" > "$LOGDIR/ntuple_vs_alphazero_X.log" 2>&1 &
CLIENT3=$!
"$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$PORT2" --role O --name az_m8 --games "$GAMES" > "$LOGDIR/ntuple_vs_alphazero_O.log" 2>&1 &
CLIENT4=$!

echo "ntuple vs rulebased2 on port $PORT3"
env CONTRAST_SERVER_PORT="$PORT3" "$CLIENT_APP" X ntuple_X5 ntuple "$GAMES" > "$LOGDIR/ntuple_vs_rulebased2_X.log" 2>&1 &
CLIENT5=$!
env CONTRAST_SERVER_PORT="$PORT3" "$CLIENT_APP" O rulebased2_O5 rulebased2 "$GAMES" > "$LOGDIR/ntuple_vs_rulebased2_O.log" 2>&1 &
CLIENT6=$!
