...
VENV_DIR="${ROOT_DIR}/.venv_alphazero8"
...
LOGDIR="${ROOT_DIR}/logs/machine8"
PORT=${PORT:-18906}
GAMES=${1:-1000}
mkdir -p "$LOGDIR"

# If previous server/client processes are still running, kill them to avoid port conflicts
echo "Cleaning up existing server/client processes for machine8 (if any)"
...
echo "Running 3 matches sequentially on ports $PORT1,$PORT2,$PORT3 for machine8"
SERVER_PIDS=()
CLIENT_PIDS=()
for i in 1 2 3; do
	case "$i" in
		1)
			P=$PORT1
			echo "Match #$i: mcts vs ntuple on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X mcts_X4 mcts "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O ntuple_O4 ntuple "$GAMES")
			LOG1="$LOGDIR/mcts_vs_ntuple_X.log"
			LOG2="$LOGDIR/mcts_vs_ntuple_O.log"
			;;
		2)
			P=$PORT2
			echo "Match #$i: ntuple vs alphazero on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X ntuple_X4 ntuple "$GAMES")
			CMD2=("$VENV_PY" "$ALPHAZERO_BOT" --host 127.0.0.1 --port "$P" --role O --name az_m8 --games "$GAMES")
			LOG1="$LOGDIR/ntuple_vs_alphazero_X.log"
			LOG2="$LOGDIR/ntuple_vs_alphazero_O.log"
			;;
		3)
			P=$PORT3
			echo "Match #$i: ntuple vs rulebased2 on port $P"
			CMD1=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" X ntuple_X5 ntuple "$GAMES")
			CMD2=(env CONTRAST_SERVER_PORT="$P" "$CLIENT_APP" O rulebased2_O5 rulebased2 "$GAMES")
